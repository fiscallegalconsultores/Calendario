...
// Versión 3.0 - Calendario Grid Compacto
...
let currentTieneEmpleados = "todos";
...
const prorrogaInfo = document.getElementById("prorrogaInfo");

// ========================= 2. DÍAS INHÁBILES Y VACACIONES ==================
const festivosFijos = ["01-01", "05-01", "09-16", "12-25"];

// Periodos vacacionales por entidad (para colorear el grid)
const vacacionesPorEntidad = {
    sat: [
        { fecha_inicio: "2025-12-18", fecha_fin: "2026-01-02", descripcion: "Vacaciones SAT: suspensión de plazos fiscales", tipo: "sat" },
...
// Inhábiles específicos adicionales
const inhabilesEspecificos = {
    sat: [],
    tfja: [],
    pjf: []
};
...
function esDiaInhabil(fecha, entidad) {
    ...
    // Verificar vacaciones (días inhábiles)
    if (entidad !== "todos" && vacacionesPorEntidad[entidad]) {
        const fechaStr = fecha.toISOString().split('T')[0];
        for (let v of vacacionesPorEntidad[entidad]) {
...
function esPeriodoVacacional(fecha, entidad) {
    if (entidad !== "todos" && vacacionesPorEntidad[entidad]) {
        const fechaStr = fecha.toISOString().split('T')[0];
        for (let v of vacacionesPorEntidad[entidad]) {
    ...
    // Si es "todos", buscar en cualquier entidad
    if (entidad === "todos") {
        for (let ent in vacacionesPorEntidad) {
            for (let v of vacacionesPorEntidad[ent]) {
                if (fechaStr >= v.fecha_inicio && fechaStr <= v.fecha_fin) return v;
            }
        }
    }
...
function actualizarInfoProrroga() {
    if (prorrogaInfo) {
        if (currentRFCcompleto && currentRFCcompleto.length >= 6) {
            const dias = obtenerDiasProrrogaRFC(currentRFCcompleto);
            const sexto = currentRFCcompleto.charAt(5);
            prorrogaInfo.innerHTML = `+${dias} día${dias !== 1 ? 's' : ''} (dígito ${sexto})`;
            prorrogaInfo.className = "text-xs font-bold text-green-700";
        } else {
            prorrogaInfo.innerHTML = "Sin RFC";
            prorrogaInfo.className = "text-xs font-bold text-slate-500";
        }
    }
}
...
function generarEventosMes() {
    ...
    // ========== SAT ==========
    if (satActivo) {
        // Pago provisional (día 17) - SOLO APLICA PRÓRROGA RFC
        ...
        eventos.push({
            ...
            descripcion: `Declaración mensual de impuestos.${aplicaProrroga ? ` Prórroga RFC aplicada: +${obtenerDiasProrrogaRFC(currentRFCcompleto)} días.` : " Sin prórroga RFC."}`
        });
    ...
    if (imssActivo) {
        ...
        // Prima de Riesgo - ÚLTIMO DÍA HÁBIL DE FEBRERO
        if (month === 1) {
            ...
            eventos.push({
                ...
                descripcion: "Determinación de prima de riesgo de trabajo basado en siniestralidad del año anterior. Último día hábil de febrero."
            });
        }
    }
    ...
    // ========== LABORAL ==========
    if (laboralActivo) {
        // PTU Personas Morales (30 mayo) - con Art. 12 CFF
        if (month === 4 && currentTipoContribuyente !== "fisica") {
            ...
            eventos.push({
                ...
                descripcion: "Reparto de utilidades (10% de utilidades fiscales) para trabajadores de empresas. Aplica prórroga por día inhábil."
            });
        }
        
        // PTU Personas Físicas (29 junio) - con Art. 12 CFF
        if (month === 5 && currentTipoContribuyente !== "moral") {
            ...
            eventos.push({
                ...
                descripcion: "Reparto de utilidades para trabajadores de personas físicas con actividad empresarial. Aplica prórroga por día inhábil."
            });
        }
        
        // Aguinaldo (20 diciembre) - con Art. 12 CFF
        if (month === 11) {
            ...
            eventos.push({
                ...
                descripcion: "Pago mínimo de 15 días de salario antes del 20 de diciembre. Aplica prórroga por día inhábil."
            });
        }
    }
    
    // ========== TFJA (días inhábiles como eventos) ==========
    if (tfjaActivo && vacacionesPorEntidad.tfja) {
        for (let v of vacacionesPorEntidad.tfja) {
            ...
            if (inicio.getFullYear() === year && inicio.getMonth() === month) {
                eventos.push({
                    ...
                    descripcion: v.descripcion + ". Durante este periodo NO corren términos procesales para demandas de nulidad."
                });
            }
        }
    }
    ...
    // ========== PJF ==========
    if (pjfActivo && vacacionesPorEntidad.pjf) {
        for (let v of vacacionesPorEntidad.pjf) {
            let inicio = new Date(v.fecha_inicio);
            if (inicio.getFullYear() === year && inicio.getMonth() === month) {
                eventos.push({
                    fecha: inicio,
                    ...
                    descripcion: v.descripcion + ". Suspensión de labores judiciales. Plazos procesales congelados."
                });
            }
        }
    }
...
// ========================= 6. RENDERIZADO DEL GRID COMPACTO ==================
function renderCalendar() {
    ...
    for (let i = 0; i < 42; i++) {
        ...
        const periodoVac = esPeriodoVacacional(dateObj, currentCalendarType);
        ...
        // Mostrar máximo 2 eventos (por tamaño compacto)
        const maxMostrar = 2;
        ...
        if (restantes > 0) {
            ...
            moreDiv.className = "more-events text-blue-600 font-medium";
    ...
    actualizarInfoProrroga();
    
    // Actualizar selectores
    document.getElementById("selectMes").value = currentMonth;
    document.getElementById("selectAnio").value = currentYear;
    ...
    modalContenido.innerHTML = `
        <div class="space-y-2">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full ${evento.clase === 'sat' ? 'bg-blue-100 text-blue-700' : evento.clase === 'imss' ? 'bg-orange-100 text-orange-700' : evento.clase === 'infonavit' ? 'bg-green-100 text-green-700' : evento.clase === 'laboral' ? 'bg-indigo-100 text-indigo-700' : 'bg-pink-100 text-pink-700'}">
                    ${evento.entidad}
                </span>
            ...
            <div class="border-t pt-2">
                <p class="text-xs text-slate-700">${evento.descripcion}</p>
            </div>
            <div class="bg-slate-50 p-2 rounded-lg text-[10px] text-slate-600">
...
function mostrarTodosEventos(eventos, dia) {
    ...
    for (let ev of eventos) {
        html += `
            <div class="p-2 bg-slate-50 rounded-lg border-l-4 border-${ev.clase === 'sat' ? 'blue-500' : ev.clase === 'imss' ? 'orange-500' : ev.clase === 'infonavit' ? 'green-500' : ev.clase === 'laboral' ? 'indigo-500' : 'pink-500'}">
...
document.getElementById("tieneEmpleados")?.addEventListener("change", (e) => {
    currentTieneEmpleados = e.target.value;
    renderCalendar();
});
...
document.getElementById("aplicarProrrogaBtn")?.addEventListener("click", () => {
    ...
    if (rfc.length === 0) {
        currentRFCcompleto = "";
        mostrarAlerta("RFC eliminado. Las fechas de pagos provisionales vuelven al día 17 sin prórroga.", "info");
    } else if (rfc.length === 12 || rfc.length === 13) {
        currentRFCcompleto = rfc;
        const dias = obtenerDiasProrrogaRFC(rfc);
        const sexto = rfc.charAt(5);
        mostrarAlerta(`RFC ${rfc} registrado. Sexto dígito: ${sexto}. Prórroga: +${dias} día${dias !== 1 ? 's' : ''} hábil${dias !== 1 ? 'es' : ''} para pagos provisionales SAT.`, "success");
    } else {
        mostrarAlerta("RFC inválido. Debe tener 12 o 13 caracteres (letras y números).", "error");
        return;
    }
    renderCalendar();
});

document.getElementById("limpiarRFCBtn")?.addEventListener("click", () => {
    currentRFCcompleto = "";
    document.getElementById("rfcCompleto").value = "";
    mostrarAlerta("RFC eliminado. Sin prórroga para pagos provisionales.", "info");
    renderCalendar();
});
